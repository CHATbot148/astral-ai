    ...
    >
      ...
      <div className={cn(
        ...
        {!isUser && !isStreaming && content && onApproveImageGeneration && parsedContent.mediaItems.length === 0 && (() => {
          ...
          const mentionsGeneration = /\b(generate|create|make|render|draw|produce|visuali[sz]e|go ahead|approval|approve|confirm)\b/i.test(content);
          const mentionsVisual = /\b(image|picture|photo|illustration|visual|artwork|wallpaper|scene|portrait|poster|render|puppy|beach|style|composition|lighting|subject)\b/i.test(content);
          const asksApproval = /(shall\s+i\s+(?:go\s+ahead\s+and\s+)?(?:generate|create|make|render|produce|draw)|would\s+you\s+like\s+me\s+to\s+(?:generate|create|make|render|produce|draw)|want\s+me\s+to\s+(?:generate|create|make|render|produce|draw)|should\s+i\s+(?:go\s+ahead\s+and\s+)?(?:generate|create|make|render|proceed)|ready\??\s*(?:say|tap|hit|approve|confirm|go ahead|generate it)|say\s+[“"']?(?:go ahead|yes|generate it|create it|make it)[”"']?|just\s+(?:say|confirm|let me know)|let\s+me\s+know\s+(?:when|if)\s+you'?re\s+ready|give\s+me\s+the\s+(?:go-?ahead|green light|nod)|confirm\s+to\s+(?:generate|proceed)|approve.{0,30}(?:generation|generate|proceed)|here'?s\s+(?:the|my)\s+plan|here\s+is\s+(?:the|my)\s+plan|here'?s\s+what\s+i'?ll\s+(?:do|create|generate)|sound\s+good\??|does\s+(?:this|that)\s+(?:sound|work)|ok(?:ay)?\s+to\s+(?:proceed|generate)|only\s+(?:create|generate)\s+it\s+after\s+you\s+say|tap.{0,24}approve|hit\s+approve)/im.test(content);
          const alreadyGenerated = /\b(here'?s your (?:freshly )?(?:generated )?(?:image|picture|photo)|here is your (?:generated )?(?:image|picture|photo)|i'?ve (?:just )?generated|generation request (?:is|has been) done|should see it pop up|momentarily)\b/i.test(content);
          if ((!mentionsGeneration && !mentionsVisual) || !asksApproval || alreadyGenerated) return null;
